...
async function isReallyOnline() {
  ...
  try {
    await fetch("https://fzkpmptsnnkafaaeqhnf.supabase.co/functions/v1/get-config", {
      method: "HEAD",
      cache: "no-store",
...
export async function requireSession() {
  ...
  if (cached && cached.uid && cacheAge < REFRESH_MS) {
    _resolved = true;
    // Silently re-verify + refresh in background without blocking render
    maybeFreshRefresh(cached.uid);
    return cached;
  }
