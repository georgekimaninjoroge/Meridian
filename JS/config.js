 ...
 *
 * OFFLINE FIX: config now cached in localStorage (survives tab close)
 * instead of sessionStorage (cleared on tab close = cold fetch every restart).
 */
...
export async function getFirebaseConfig() {
  ...
  // localStorage survives tab close -- instant hit offline
  const cached = localStorage.getItem("meridian_fb_config");
  if (cached) { _firebaseConfig = JSON.parse(cached); return _firebaseConfig; }
  // No cache + offline = fail fast instead of hanging on fetch timeout
  if (!navigator.onLine) throw new Error("Offline: no firebase config cache yet");
  const res = await fetch(CONFIG_ENDPOINT);
  if (!res.ok) throw new Error(`get-config failed: ${res.status}`);
  const data      = await res.json();
  _firebaseConfig = data.firebase;
  localStorage.setItem("meridian_fb_config", JSON.stringify(_firebaseConfig));
...
export async function getConfig(idToken) {
  ...
  // localStorage survives tab close -- instant hit offline
  const cached = localStorage.getItem("meridian_config");
  if (cached) {
  ...
  localStorage.setItem("meridian_config", JSON.stringify(_fullConfig));
  return _fullConfig;
}

export function clearConfig() {
  _firebaseConfig = null; _fullConfig = null;
  localStorage.removeItem("meridian_fb_config");
  localStorage.removeItem("meridian_config");
}